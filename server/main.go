	...
	app.Commands = []*cli.Command{
		...
		},
		{
			...
			Action: func(c *cli.Context) error {
				...
				// Build a shared metrics handler so both the etcd dynconfig client
				// and the Temporal server emit into the same Prometheus registry
				// on the same HTTP listener. Passing it via WithCustomMetricsHandler
				// prevents the server from starting a second conflicting listener.
				...
				dcClient, err := etcddynconfig.NewClient(c.Context, etcdClient, etcdCfg.GlobalKeyPrefix, logger,
					metricsHandler.WithTags(metrics.StringTag("service_name", strings.Join(services, ","))),
				)
				if err != nil {
					return cli.Exit(fmt.Sprintf("Unable to create etcd dynconfig client: %v.", err), 1)
				}
				defer dcClient.Stop()
