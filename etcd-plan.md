...
- [Plan: etcd-backed dynamic config](#plan-etcd-backed-dynamic-config)
  ...
  - [What happens when you push a bad value via etcdctl](#what-happens-when-you-push-a-bad-value-via-etcdctl)
    - [Three ways a bad write can go wrong](#three-ways-a-bad-write-can-go-wrong)
      - [1. Invalid YAML — broken structure, missing dash, bad indentation](#1-invalid-yaml--broken-structure-missing-dash-bad-indentation)
      - [2. Valid YAML, wrong type (string where int expected, etc.)](#2-valid-yaml-wrong-type-string-where-int-expected-etc)
      - [3. Typo in key name](#3-typo-in-key-name)
    - [Summary table](#summary-table)
    - [What to do about it — planned improvement](#what-to-do-about-it--planned-improvement)
  - [Managing config across separate clusters (prod / dev / loadtest)](#managing-config-across-separate-clusters-prod--dev--loadtest)
    ...
    - [Deleting a key (revert to compiled-in default)](#deleting-a-key-revert-to-compiled-in-default)
...
replace (
  go.temporal.io/server                          => ${HOME}/devel/temporal/temporal
  github.com/temporalio/temporal-etcd-dynconfig  => ${HOME}/devel/temporal-etcd-dynconfig
)
```
